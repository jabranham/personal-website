...
There are four big text editors out there: Atom, Emacs, Sublime Text,
and vim. Sublime is neither free nor libre. Atom is open source, but
...
that I use it for. So what exatly is emacs? 
...
[git](http://www.git-scm.com/). In this blog post, I'll briefly
...
is to install emacs on your system. 
...
* **Linux**: For Ubuntu, you can just do `sudo apt-get install emacs24`
  for the latest version as of this writing.[^ppa] Other linux
...
`.emacs` file a little bit. You can do `M-x package-list-packages` to
...
way (no idea why), so I download and install it from Ubuntu's
...
2. RefTeX should automatically find your .bib file through the
   `\bibliography{}` LaTeX command. So if you've set up your
...
citation is `C-c [`. This will bring up a menu that looks something like this:
...
want. Once I'm on it, hitting enter will automatically insert the selection into my tex file.
...
whole bunch of options. From the `magit-status` screen, you can press
`b` to see options relating to branching. `z` will pull up stashing
options. There's a nice cheatsheet of the most common actions for
